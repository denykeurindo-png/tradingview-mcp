...
const s = fs.readFileSync(path.join(process.cwd(), 'scratch/remove_bs.py'), 'utf8');
const conn = new Client();
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    const ws = sftp.createWriteStream('/tmp/remove_bs.py');
    ws.write(s); ws.end();
    ws.on('close', () => conn.exec('python3 /tmp/remove_bs.py', (e, st) => {
