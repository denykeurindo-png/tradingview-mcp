...
async function testApi(apiUrl, name) {
  try {
    ...
    console.log(`[${name}] Last VZO:`, vzo[vzo.length - 1], 'Signal:', signal[signal.length - 1]);
  } catch (e) {
...
async function run() {
  const spotUrl = 'https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=15m&limit=1000';
  const futuresUrl = 'https://fapi.binance.com/fapi/v1/klines?symbol=BTCUSDT&interval=15m&limit=1000';
  await testApi(spotUrl, 'Spot (api.binance.com)');
  await testApi(futuresUrl, 'Futures (fapi.binance.com)');
}
run();
