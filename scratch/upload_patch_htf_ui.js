...
const s = fs.readFileSync(path.join(process.cwd(), 'scratch/patch_htf_ui.py'), 'utf8');
const conn = new Client();
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    const ws = sftp.createWriteStream('/tmp/patch_sweep_ui.py');
    ws.write(s); ws.end();
    ws.on('close', () => conn.exec('python3 /tmp/patch_sweep_ui.py', (e, st) => {
