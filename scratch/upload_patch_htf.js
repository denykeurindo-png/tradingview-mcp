...
const s = fs.readFileSync(path.join(process.cwd(), 'scratch/patch_htf.py'), 'utf8');
const conn = new Client();
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    const ws = sftp.createWriteStream('/tmp/patch_htf3d.py');
    ws.write(s); ws.end();
    ws.on('close', () => conn.exec('python3 /tmp/patch_htf3d.py', (e, st) => {
