...
const s = fs.readFileSync(path.join(process.cwd(), 'scratch/patch_prob.py'), 'utf8');
const conn = new Client();
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    const ws = sftp.createWriteStream('/tmp/patch_sweep.py');
    ws.write(s); ws.end();
    ws.on('close', () => conn.exec('python3 /tmp/patch_sweep.py', (e, st) => {
